      ...
      <div className="max-w-xl bg-gray-100 shadow-md rounded px-8 pt-6 pb-8 mb-2">
        ...
        <form onSubmit={handleSubmit} noValidate className="bg-white shadow-md rounded px-8 pt-6 pb-4">
          <div className="mb-4">
            ...
            />
            {userError && <p className="text-red-500 text-xs italic -mt-3 mb-2">{userError}</p>}
          </div>
          <div className="mb-4">
            <Input 
              id="email" 
            ...
            />
            {emailError && <p className="text-red-500 text-xs italic -mt-3 mb-2">{emailError}</p>}
          </div>
          <div className="mb-4 relative">
            ...
            className="absolute right-3 top-11.5 transform -translate-y-1/2 text-gray-600 hover:text-gray-900"
            >
            {showPassword ? <FiEyeOff size={18} /> : <FiEye size={18} />}
            </button>
            {passError && <p className="text-red-500 text-xs italic -mt-3 mb-2">{passError}</p>}
            <ValidadorSenha password={password} visible={isPasswordFocused} />
          </div>
          <div className="mb-4">
            <Input
              id="confirmPassword"
          ...
          {!isPasswordFocused && <p className="opacity-100 italic block">Ja tem uma conta? <a className="text-sky-500 hover:underline" href="/login">Login</a></p>}
          <Button
            type="submit"
            disabled={isLoading}
            className="cursor-pointer bg-sky-500 hover:bg-sky-600 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline w-full mt-2"
          >
